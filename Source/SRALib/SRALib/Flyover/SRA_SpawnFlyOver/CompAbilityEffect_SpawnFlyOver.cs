...
namespace SRA
{
    public class CompAbilityEffect_SpawnFlyOver : CompAbilityEffect
    {
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            try
            {
                SRALog.Debug($"FlyOver skill activated by {parent.pawn.Label} at position {parent.pawn.Position}");
                SRALog.Debug($"Target cell: {target.Cell}, Dest: {dest.Cell}");
                ...
                SRALog.Debug($"Final positions - Start: {startPos}, End: {endPos}");
                
                // 验证位置是否有效
                if (!startPos.InBounds(parent.pawn.Map))
                {
                    SRALog.Debug($"Start position {startPos} is out of bounds, adjusting to map center");
                    startPos = parent.pawn.Map.Center;
                }
                
                if (!endPos.InBounds(parent.pawn.Map))
                {
                    SRALog.Debug($"End position {endPos} is out of bounds, adjusting to map center");
                ...
                if (startPos == endPos)
                {
                    SRALog.Debug($"FlyOver start and end positions are the same: {startPos}. Adjusting end position.");
            ...
            {
                SRALog.Debug($"Error spawning fly over: {ex}");
            }
        }
        ...
        private List<IntVec3> CalculateStrafingImpactCells(IntVec3 targetCell, Vector3 flightDirection)
        {
            ...
            SRALog.Debug($"Strafing Area: Calculated {cells.Count} impact cells centered at {targetCell}");
            return cells;
        }
        ...
        private List<IntVec3> PreprocessStrafingTargets(List<IntVec3> potentialTargets, float fireChance)
        {
            ...
            SRALog.Debug($"Strafing Preprocess: {confirmedTargets.Count}/{potentialTargets.Count} cells confirmed after min/max adjustment.");
        ...
        {
            ThingDef flyOverDef = Props.flyOverDef ?? DefDatabase<ThingDef>.GetNamedSilentFail("SRA_HiveCorvette");
            if (flyOverDef == null)
            {
                SRALog.Debug("No fly over def specified for ground strafing fly over");
                return;
            }
            ...
            if (strafingComp != null)
            {
                ...
                if (potentialTargetCells.Count > 0)
                {
                    ...
                    {
                        SRALog.Debug("No confirmed target cells after preprocessing!");
                    }
                }
                else
                {
                    SRALog.Debug("No potential target cells calculated for ground strafing!");
                }
            }
            else
            {
                SRALog.Debug("FlyOver def does not have CompGroundStrafing component!");
            }
        }
        ...
        private void CreateSectorSurveillanceFlyOver(IntVec3 startPos, IntVec3 endPos)
        {
            ...
            if (flyOverDef == null)
            {
                SRALog.Debug("No fly over def specified for sector surveillance fly over");
                return;
            }
            ...
            SRALog.Debug($"SectorSurveillance FlyOver created: {flyOver} from {startPos} to {endPos}");
        }
        ...
        private void CalculatePerpendicularPath(LocalTargetInfo target, out IntVec3 startPos, out IntVec3 endPos)
        {
            ...
            SRALog.Debug($"Calculating perpendicular path: Caster={casterPos}, Target={targetPos}");
            ...
            SRALog.Debug($"Perpendicular path: {startPos} -> {targetPos} -> {endPos}");
        }
        ...
        private IntVec3 FindMapEdgeInDirection(Map map, IntVec3 fromPos, Vector3 direction)
        {
            ...
            SRALog.Debug($"Could not find map edge in direction {direction}, using random edge");
        ...
        private IntVec3 CalculateEndPosition(LocalTargetInfo target, IntVec3 startPos)
        {
            ...
            switch (Props.endPosition)
            {
                ...
                case EndPosition.RandomMapEdge:
                    ...
                    SRALog.Debug($"Random map edge selected as end position: {endPos}");
                    break;
        ...
        private IntVec3 GetOppositeMapEdgeThroughCenter(Map map, IntVec3 startPos)
        {
            ...
            if (toCenter == Vector3.zero)
            {
                ...
                SRALog.Debug($"Using random direction to center: {toCenter}");
            }
            
            Vector3 fromCenter = toCenter;
            IntVec3 oppositeEdge = GetMapEdgePositionFromCenter(map, fromCenter);
            
            SRALog.Debug($"Found opposite edge through center: {oppositeEdge}");
            return oppositeEdge;
        }
        ...
        private IntVec3 GetMapEdgePositionFromCenter(Map map, Vector3 direction)
        {
            ...
            for (int i = 1; i <= maxDist; i++)
            {
                ...
                if (!testPos.InBounds(map))
                {
                    ...
                    SRALog.Debug($"Found map edge from center: {edgePos} (direction: {direction}, distance: {i})");
                    return edgePos;
                }
            }
            
            SRALog.Debug("Could not find map edge from center, using random edge");
            return GetRandomMapEdgePosition(map);
        }
        ...
        private IntVec3 GetMapEdgePosition(Map map, Vector3 direction)
        {
            ...
            {
                direction = new Vector3(Rand.Range(-1f, 1f), 0, Rand.Range(-1f, 1f)).normalized;
                SRALog.Debug($"Using random direction: {direction}");
            }
            ...
            for (int i = 1; i <= maxDist; i++)
            {
                ...
                {
                    IntVec3 edgePos = FindClosestValidPosition(testPos, map);
                    SRALog.Debug($"Found map edge position: {edgePos} (direction: {direction}, distance: {i})");
                    return edgePos;
                }
            }
            
            SRALog.Debug("Could not find map edge in direction, using random edge");
            return GetRandomMapEdgePosition(map);
        }
        ...
        private IntVec3 GetRandomMapEdgePosition(Map map)
        {
            ...
            SRALog.Debug($"Random map edge position: {edgePos}");
            return edgePos;
        }
        ...
        private IntVec3 GetFixedDistancePosition(IntVec3 startPos, IntVec3 targetPos)
        {
            ...
            SRALog.Debug($"Fixed distance position: {endPos} (from {startPos}, distance: {Props.flyOverDistance})");
            return endPos;
        }
        ...
        {
            ThingDef flyOverDef = Props.flyOverDef ?? DefDatabase<ThingDef>.GetNamedSilentFail("SRA_HiveShip");
            if (flyOverDef == null)
            {
                SRALog.Debug("No fly over def specified for standard fly over");
                return;
            }
            ...
            SRALog.Debug($"Standard FlyOver created: {flyOver} from {startPos} to {endPos}");
        }
